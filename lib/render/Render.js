...
module.exports =  class Render {
    ...
    static rules = this.defaultRules
    // static changeRules = (ruleName,func) => {
        
    // }
    // 利用箭头函数特性 将当前实例的this绑定给该函数
    static tokens2Html = (tokens)=>{
        let result = ''
    ...
    static heading_open(token){
        const headingType = token.tag
        return '<'+ headingType +'>'
    }
    static heading_close(token){
        const headingType = token.tag
        return '</'+ headingType +'>'
    }
    static paragraph_open(token){
        return '<p>'
    }
    static paragraph_close(token){
        return '</p>'
    }
    static inline(token){
